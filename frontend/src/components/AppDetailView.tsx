      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            {prettyAppName(appName)} — {isViewingToday ? 'Today' : selectedKey}
          </h1>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <motion.div variants={sectionVariants} className="grid grid-cols-3 gap-4">
            ...
            <StatCard label={isViewingToday ? 'Today' : 'Selected day'} value={formatHoursMinutes(selectedTotal)} big />
          ...
          <motion.div variants={sectionVariants}>
            <h2 className="mb-2 text-sm font-semibold text-on-surface-variant">Daily usage</h2>
...
function TrendCard({ delta }: { delta: number }) {
  ...
  return (
    <div className="rounded-lg bg-surface-container p-4">
      <p className="text-xs text-on-surface-variant">vs yesterday</p>
