...
export const HourlyUsageChart = memo(function HourlyUsageChart({ minutesByHour }: HourlyUsageChartProps) {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg bg-surface-container p-4">
      <div className="flex flex-1 items-end gap-1">
        {minutesByHour.map((minutes, hour) => (
          <div
            key={hour}
            className="flex h-full flex-1 items-end"
            title={`${String(hour).padStart(2, '0')}:00 — ${Math.round(minutes)}m`}
          >
            <motion.div
              className={`w-full rounded-sm ${minutes > 0 ? 'bg-primary' : 'bg-surface-container-high'}`}
              initial={{ height: '1%' }}
              animate={{ height: `${Math.max(1, (minutes / max) * 100)}%` }}
